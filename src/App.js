...
import { getMergeSortAnimation } from './Components/SortingAlgo';

const numberOfArrayBars = 80;
const minValue = 10;
const maxValue = 600;
const PRIMARY_COLOR = "#B2A4FF"
const SECONDARY_COLOR = "#FFB4B4"
const ANIMAITON_SPEED = 10
...
function App() {
  ...
  const mergeSort = () => {
    ...
    for(let i = 0; i< animation.length; i++){
      const arrayBars = document.querySelector('#bar');
      const isColorChange = i%3 !== 2;

      if(isColorChange){
        const [barOneIndex, barTwoIndex ] = animation[1];
    ...
    <div className="h-screen">
      <div className="px-10 py-5 bg-red-100 justify-between flex">
        ...
        <div className="space-x-5">
          <button onClick={mergeSort} className="w-fit bg-pink-500 text-white font-bold px-3 py-2  bg-red rounded ">Merge Sort</button>
